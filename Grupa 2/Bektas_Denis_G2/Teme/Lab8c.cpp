...
#include <iostream> /*  < librarie lipsa */
#include <string.h> /*  < librarie lipsa */
#include <cstdlib>
#include <cstring>
...
int main() {
    ...
    for (int i = 0; i < size; ++i) { /* am modificat i < size din i <= size */
        ...
        if (arrMalloc[i] == nullptr) {
            ...
            free(arrMalloc); /* ; lipsa dupa functie */
            return 1;
        }
    ...
    for (int i = size; i < newSize; ++i) {
        ...
        if (arrRealloc[i] == nullptr) { /* variabila lipsa pentru realloc */
            std::cout << "Memory allocation failed for string " << i << " using malloc!" << std::endl;

            for (int j = 0; j < i; ++j) { /* ; lipsa in for */
                free(arrRealloc[j]);
            }
            free(arrRealloc);
            return 1;
...
/* programul este asemanator celui de la 1b, doar ca in acest program putem utiliza si siruri de caractere
Se introduce o dimensiune pentru vectorul initial, apoi elemntele sale, dupa care este reinlocuit cu un vector nou, unde se introduc si elemntele noi */
