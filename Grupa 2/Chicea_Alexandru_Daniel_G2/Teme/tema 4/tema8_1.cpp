...
int main() {
    int size;
    std::cout << "Enter the size of the array: "; /* Eroare de sintaxa - Lipseste ":" */
    std::cin >> size;
    ...
    int *arrCalloc = static_cast<int *>(calloc(size, sizeof(int))); /* Eroare de sintaxa - Lipseste "*" */
    ...
    if (arrRealloc == nullptr) { /* Eroare de logica - Lipseste "=" */
    ...
    free(arrRealloc);
    free(arrCalloc); /* Eroare de sintaxa - Lipseste ";" */
...
/*  Programul trece intr-un mod foarte simplu prin modul in care functioneaza malloc,calloc,realloc. */
