...
#include <iostream>  /* Lipseste libraria iostream */
#include <cstdlib>
#include <cstring>
...
int main() {
    ...
    for (int i = 0; i <= size; ++i) {
        ...
        if (arrMalloc[i] == nullptr) {
            ...
            free(arrMalloc); /* Lipseste caracterul ";" */
            return 1;
        }
    ...
    for (int i = size; i < newSize; ++i) {
        ...
        if (arrRealloc[i] == nullptr) { /* Lipseste "i" */
            std::cout << "Memory allocation failed for string " << i << " using malloc!" << std::endl;

            for (int j = 0; j < i; ++j) { /* Lipseste ";" */
                free(arrRealloc[j]);
            }
            free(arrRealloc);
            return 1;
        }
        std::cin.getline(arrRealloc[i], bufferSize); /* Lipseste elementul "." din cin.getline()  */
    }
