...
#include<iostream> //lipsea libraria iostream
#include <cstdlib> 
#include <cstring> 
...
int main() {
    ...
    for (int i = 0; i <size; ++i) {   //i ajungea la size inclusiv
    ...
    for (int i = size; i < newSize; ++i) {
        ...
        if (arrRealloc[i] == nullptr) {  //lipsesa i intre parantezele patrate
            std::cout << "Memory allocation failed for string " << i << " using malloc!" << std::endl;

            for (int j = 0; j < i; ++j) {   //lipseau ";" la j<i
                free(arrRealloc[j]);
            }
            free(arrRealloc); 
            return 1;
        }
        std::cin.getline(arrRealloc[i], bufferSize);  //lipsea "." intre cin si getline
    }
